...
The UI is intended for a single trusted operator on the local machine. It uses named AWS profiles and preserves the same read-only preview plus explicit restore confirmation used by the CLI. The Workspace tab lets the operator create or open a persistent local folder identified by a versioned `.quicksight-workspace.json` marker; configs, manifests, overrides, backup outputs, reports, History, and workspace-file selectors then survive Streamlit restarts. Because Streamlit has no native server-side folder picker, the local path is entered explicitly and validated against symlink/reparse-point and marker rules. Backup and restore configurations can be uploaded and edited inline after safe YAML/JSON normalization, loaded directly from the workspace folder, or started from validated JSON templates; identity mappings are edited in `restore.identity_mappings`, and optional API-native overrides have the same workflow. Edited files can be saved in JSON or YAML before execution. ZIP export/restore remains available only as an optional portability feature; plans and temporary files are excluded and regenerated through a fresh preview. Backup manifests remain immutable because they are authoritative restore evidence. Do not bind the UI to a public interface without adding an authenticated deployment boundary.
