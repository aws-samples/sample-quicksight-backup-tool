...
Backup authentication is selected with the **AWS profile** dropdown in the Backup tab. Restore authentication comes from the restore configuration: `source_backup.auth.profile` reads S3/DynamoDB backup artifacts, while `target.auth.profile` performs Quick Sight imports and target identity checks. The UI passes only profile names to Boto3; it never stores access keys, secret keys, passwords, or SSO tokens in the workspace.

Backup validation/run and restore preview/execute use an operation guard. Before a controller starts, the interactive tabs are replaced with a progress-only view so editor, workspace, and archive actions cannot interrupt tracking. Progress and completion/failure summaries are retained in session state and reconstructed after ordinary widget reruns; the UI returns to the Backup or Restore tab that launched the operation and reloads saved operation inputs as workspace files. Keep the browser page open during an active operation—a full browser refresh or Streamlit server restart is not an operation-resume mechanism.

Do not bind the UI to a public interface without adding an authenticated deployment boundary.
