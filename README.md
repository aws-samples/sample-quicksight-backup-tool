...
The UI is intended for a single trusted operator on the local machine. It uses named AWS profiles, stores uploaded/generated files in a random session directory under the operating-system temp folder, and preserves the same read-only preview plus explicit restore confirmation used by the CLI. Backup and restore configurations can be uploaded and edited inline after safe YAML/JSON normalization, or started from validated JSON templates; identity mappings are edited in `restore.identity_mappings`, and optional API-native overrides have the same upload/edit/template workflow. Edited files can be saved in JSON or YAML before execution. The Workspace tab creates a bounded portable ZIP containing configs, manifests, overrides, backup outputs, and reports; restoring it loads those files into a new isolated session and makes them selectable throughout the UI. Plans and temporary files are excluded and regenerated through a fresh preview. Backup manifests remain immutable because they are authoritative restore evidence. Do not bind the UI to a public interface without adding an authenticated deployment boundary.
